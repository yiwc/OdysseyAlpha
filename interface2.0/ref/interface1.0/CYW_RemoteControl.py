...
class operator():
    ...
    def send_server_start(self):
        self.main_thread=Thread(target=self.send_thread)
    ...
    def receive_server_start(self):
        self.main_thread = Thread(target=self.receive_thread)
        print('iamhere')
        self.main_thread.start()
...
class controller():
    ...
    def do_go_to_xy_then_pick(self, x, y):
        ...
        target_pos_ls.append([691, -0, 312, -0.76, 0.058, -1.9, 150])  # 6
        target_pos_ls.append([691, -0, 83, -0.76, 0.058, -1.9, 150])  # 7
