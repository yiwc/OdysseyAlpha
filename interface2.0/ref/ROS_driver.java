...
public class ROS_driver_v2 extends RoboticsAPIApplication {
    ...
    private Tool tool;
    private RobotiqGrip3IOGroup gripper;
    ...
    public void initialize() {
        ...
        gripper = new RobotiqGrip3IOGroup(controller);
    }
...
// Gripper operations
// --------------------------------------------------------------------------------------
        } else if (command.compareToIgnoreCase("get gripper position force") == 0){
            try {

                int position = gripper.getFingerPos();
                int force = gripper.getForce();
                return String.valueOf(position) + " " + String.valueOf(force);

            } catch (Exception e){
                return "error";
            }

        } else if (command.compareToIgnoreCase("gripper move") == 0){
            try {

                int force = Integer.parseInt(parameters[1])
                int mode = Integer.parseInt(parameters[3])
                if (gripper.getForce() != force) gripper.setForce(force);
                if (gripper.getActReq() != mode) gripper.setActReq(mode);
                gripper.setSpeed(Integer.parseInt(parameters[2]));
                gripper.setPosReq(Integer.parseInt(parameters[0]));
