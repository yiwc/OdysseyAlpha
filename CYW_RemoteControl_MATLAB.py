...
class MATLABs():
    def __init__(self):
        self.PARAMETERS_FILE_PATH="MATLAB/paras_optimal_1018.txt"
        self.WF_FILE_PATH="MATLAB/wf.txt"
        self.ParametersFile=open(self.PARAMETERS_FILE_PATH,"r")
        self.WfFile=open(self.WF_FILE_PATH,"r")
        self.Wf_raw=self.WfFile.read()
        self.Paras_raw=self.ParametersFile.read()
        self.Paras=self.GetParas()
    def GetParas(self):
        #a[joints][l]
        #b[joints][l]
        a=[[] for i in range(7)]
        b=[[] for i in range(7)]
        # print(self.Wf_raw)
        wf=float(self.Wf_raw)*np.pi
        wf=format(wf,".7f")
        pars={"a":a,"b":b,"wf":wf}
        pars_raw=self.Paras_raw.split("\n")
        for row in range(len(pars_raw)):
            # print("row:",row)
            items=pars_raw[row].split("\t")
            for joint_index in range(len(items)):
                item=items[joint_index]
                item_f=float(item)
                item_f=format(item_f,".7f")
                item=str(item_f)
            # for item in items:
                if(row%2==1):
                    b[joint_index].append(item)
                else:
                    a[joint_index].append(item)
        # pars=self.Paras_raw.split("\n")
        # print(pars)
        return pars
class funcs():
    @staticmethod
...
class operator():
    def __init__(self,global_db,MATLABs_OBJ):
        self.mode=0# 0,haert and waiting; 1,sending paras;2,download paras
        self.mt=MATLABs_OBJ
    ...
    def send_thread(self):
        ...
        while (1):
            if(self.mode==0):
                sending_cont=self.FormSendingCont()
                # a = 1
                sending_msg = bytes(sending_cont, encoding="utf8")

                print(sending_msg)
                self.send_cont(sending_msg)
                time.sleep(0.02)
            elif(self.mode==1):#sending paras;
                sending_cont=self.FormSendingCont()
                # a = 1
                sending_msg = bytes(sending_cont, encoding="utf8")
                self.send_cont(sending_msg)
                ...
                time.sleep(1)
                pass
            elif(self.mode==2):#send blank;
                sending_cont=self.FormSendingCont()
                # a = 1
                sending_msg = bytes(sending_cont, encoding="utf8")
                self.send_cont(sending_msg)
                time.sleep(0.1)
            elif(self.mode==3):#send blank
                sending_cont=self.FormSendingCont()
                # a = 1
                sending_msg = bytes(sending_cont, encoding="utf8")
                self.send_cont(sending_msg)
    ...
    def FormSendingCont(self):
        # 0 xxxxx xxx xx x xx  xx x x x
        # 1 x x x xxx x x xx
        # 2 xx  xx x x
        if(self.mode==0):#heart
            sendStr=str(self.mode)+" "
            pass
        elif(self.mode==1):#sending paras;
            sendStr=str(self.mode)+" "
            ab_paras=""
            for a_para_joints in self.mt.Paras["a"]:
                for a_para in a_para_joints:
                    ab_paras+=a_para+" "
            # sendStr+=ab_paras
            for b_para_joints in self.mt.Paras["b"]:
                for b_para in b_para_joints:
                    ab_paras+=b_para+" "
            sendStr+=ab_paras
            wf=self.mt.Paras["wf"]
            sendStr+=str(wf)+" "
    ...
    def receive_server_start(self):
        ...
        pass
    def write_list(self,name,list):
        print("save file ing....",name)
        with open("MATLAB/"+name+'.txt', 'w') as filehandle:
            for listitem in list:
                filehandle.write('%s\n' % listitem)
    def read_list(self,name):
        res = []
        # open file and read the content in a list
        with open("MATLAB/"+name+'.txt', 'r') as filehandle:
    ...
    def receive_thread(self):
        while True:
            ...
            while True:
                ...
                data, client = self.sock.recvfrom(200000)
                ...
                # print("Received floats",self.global_db.r_iiwaFltLs[:])
                mode=self.global_db.r_iiwaFltLs[0]
                if(mode==0):
                    pass
                elif(mode==1):
                    data=self.global_db.r_iiwaFltLs[1]
                    if(data==1):
                        self.mode=2
                        print('iiwa Finished Para Set!')
                    pass
                elif(mode==2):
                    data = self.global_db.r_iiwaFltLs[1]
                    if(data==1):
                    ...
                    status=int(self.global_db.r_iiwaFltLs[1])
                    if(status==1):
                        # print(self.global_db.r_iiwaFltLs)
                        if(len(self.global_db.r_iiwaFltLs)>6):
                            data=self.global_db.r_iiwaFltLs[6:]
                            start_index=int(self.global_db.r_iiwaFltLs[4])
                            end_index=int(self.global_db.r_iiwaFltLs[5])
                            max_length=int(self.global_db.r_iiwaFltLs[3])
                            data_type=int(self.global_db.r_iiwaFltLs[2])
                            if(data_type==1):
                                print("Receiving Torque Data")
                                self.global_db.MT_torque_data+=data
                            elif(data_type==2):
                                print("Receiving Torque Time Data")
                                self.global_db.MT_torque_time_data+=data
                            elif(data_type==3):
                                print("Receiving Pos Data")
                                self.global_db.MT_pos_data+=data
                            elif(data_type==4):
                                print("Receiving Pos Time Data")
                                self.global_db.MT_pos_time_data+=data
                            print("Receive Info:","max_length:",max_length," start:",start_index," end:",end_index," data:",len(data))
                    elif(status==2):
                        #Finished Transfer
                        print("Downloading Finished")
                        self.write_list("torque_data", self.global_db.MT_torque_data)
                        self.write_list("torque_time_data", self.global_db.MT_torque_time_data)
                        self.write_list("pos_data", self.global_db.MT_pos_data)
                        self.write_list("pos_time_data", self.global_db.MT_pos_time_data)
                        print("Files saves to MATLAB/")
                        break
...
class global_database():
    ...
    def __init__(self):
        ...
        self.MT_torque_data=[]
        self.MT_torque_time_data=[]
        self.MT_pos_data=[]
        self.MT_pos_time_data=[]
