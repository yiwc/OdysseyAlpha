...
class global_database():
    ...
    def __init__(self):
        ...
        #posecnn
        self.posecnn_banana_mask=np.array([])
        self.posecnn_banana_box=[0,0,0,0]
        self.DF_sensor_able=1
        self.DF_pred_matrix=np.array([])
        # self.cam_too_close_error=1

        self.cam_image=np.array([])
        self.cam_depth=None
        self.cam_f=0
    ...
    def get_target_cam_data_list(self,name):
        if(name=="cube"):
            ...
            list=[self.cam_cube_updated,self.r2(self.cam_cube_x),self.r2(self.cam_cube_y),self.r2(self.cam_cube_z)]
        elif(name=="iiwa"):
            list=[self.cam_iiwa_updated,self.r2(self.cam_iiwa_x),self.r2(self.cam_iiwa_y),self.r2(self.cam_iiwa_z)]
        elif(name=="hand"):
            list=[self.cam_hand_updated,self.r2(self.cam_hand_x),self.r2(self.cam_hand_y),self.r2(self.cam_hand_z)]
        elif(name=="bottle"):
            list=[self.cam_bottle_updated,self.r2(self.cam_bottle_x),self.r2(self.cam_bottle_y),self.r2(self.cam_bottle_z)]
        else:
    ...
    def publish_posecnn_banana_mask(self, mask):
        self.posecnn_banana_mask = mask

    def publish_posecnn_banana_box(self, box_list):
        self.posecnn_banana_box = box_list

    def get_cam_image(self):
        return self.cam_image

    def get_cam_image_with_mask(self):
        if(self.posecnn_banana_mask.size==0):
            # print("posecnn_banana_mask.size ==0")
            return self.cam_image
        else:

            # print("posecnn_banana_mask.size ==,",self.posecnn_banana_mask.size)
            img=self.cam_image[:]
            mask=self.posecnn_banana_mask[:]
            mask=mask.transpose([1,2,0])
            # img[:,:,0]=img[:,:,0]+mask[:,:,0]*100
            # img[mask[:,:,0]]=100
            # img[:,:,2][mask.astype(np.bool)[:,:,0]]=255
            # img[:,:,1][mask.astype(np.bool)[:,:,0]]=10
            img[:,:,2]=mask[:,:,0]*255
            img[:,:,3]=mask[:,:,0]*255
            return img
        return self.cam_image
